...
enum class Endian { None, Little, Big };
...
struct NormalizedSchema {
    ...
    std::string dialect = "binmorph.schema.v1";
    Endian defaultEndian = Endian::None;
...
bool primitiveInfo(const std::string& typeName, FieldKind& kind, std::uint64_t& size) {
    if (typeName == "u8") { kind = FieldKind::UnsignedInteger; size = 1; return true; }
    if (typeName == "u16") { kind = FieldKind::UnsignedInteger; size = 2; return true; }
    if (typeName == "u32") { kind = FieldKind::UnsignedInteger; size = 4; return true; }
    if (typeName == "u64") { kind = FieldKind::UnsignedInteger; size = 8; return true; }
    if (typeName == "i8") { kind = FieldKind::SignedInteger; size = 1; return true; }
    if (typeName == "i16") { kind = FieldKind::SignedInteger; size = 2; return true; }
    if (typeName == "i32") { kind = FieldKind::SignedInteger; size = 4; return true; }
    if (typeName == "i64") { kind = FieldKind::SignedInteger; size = 8; return true; }
    if (typeName == "bytes") { kind = FieldKind::Bytes; size = 0; return true; }
    if (typeName == "ascii") { kind = FieldKind::Ascii; size = 0; return true; }
    return false;
}
...
NormalizedSchema compileSchema(const Json& document, std::vector<Diagnostic>& diagnostics) {
    ...
    if (document.type != JsonType::Object) {
        addDiagnostic(diagnostics, "BM_SCHEMA_ROOT_OBJECT", "schema document must be a JSON object", "$");
        return schema;
    }
    ...
    if (getString(document, "dialect", value)) {
        ...
        if (schema.dialect != "binmorph.schema.v1") {
            addDiagnostic(diagnostics, "BM_SCHEMA_DIALECT_UNSUPPORTED", "only binmorph.schema.v1 is supported", "$.dialect");
        }
    }
    if (getString(document, "endianness", value)) {
        schema.defaultEndian = parseEndian(value);
        if (schema.defaultEndian == Endian::None) {
            addDiagnostic(diagnostics, "BM_SCHEMA_ENDIAN_INVALID", "endianness must be 'little' or 'big'", "$.endianness");
        }
    }
    ...
    if (const Json* root = document.get("root")) {
        if (root->type != JsonType::Object) {
            addDiagnostic(diagnostics, "BM_SCHEMA_ROOT_INVALID", "root must be an object", "$.root");
        } else {
            std::string rootType;
            if (getString(*root, "type", rootType) && rootType != "struct") {
                addDiagnostic(diagnostics, "BM_SCHEMA_ROOT_TYPE_UNSUPPORTED", "root.type must be 'struct'", "$.root.type");
            }
    ...
    if (fieldsNode == nullptr || fieldsNode->type != JsonType::Array) {
        addDiagnostic(diagnostics, "BM_SCHEMA_FIELDS_REQUIRED", "schema requires a fields array", fieldsPath);
        return schema;
    }
    ...
    for (std::size_t i = 0; i < fieldsNode->array.size(); ++i) {
        ...
        if (fieldJson.type != JsonType::Object) {
            addDiagnostic(diagnostics, "BM_SCHEMA_FIELD_OBJECT", "field must be an object", fieldPath);
            continue;
        }
        ...
        if (!getString(fieldJson, "name", field.name) || field.name.empty()) {
            addDiagnostic(diagnostics, "BM_SCHEMA_FIELD_NAME_REQUIRED", "field.name must be a non-empty string", fieldPath + ".name");
            continue;
        }
        if (!getString(fieldJson, "type", field.typeName) || field.typeName.empty()) {
            addDiagnostic(diagnostics, "BM_SCHEMA_FIELD_TYPE_REQUIRED", "field.type must be a non-empty string", fieldPath + ".type");
            continue;
        }
        if (!primitiveInfo(field.typeName, field.kind, field.size)) {
            addDiagnostic(diagnostics, "BM_SCHEMA_FIELD_TYPE_UNSUPPORTED", "unsupported field type: " + field.typeName, fieldPath + ".type");
            continue;
        }
        ...
        if (field.kind == FieldKind::Bytes || field.kind == FieldKind::Ascii) {
            if (!getUint64(fieldJson, "size", explicitSize) || explicitSize == 0) {
                addDiagnostic(diagnostics, "BM_SCHEMA_FIELD_SIZE_REQUIRED", "bytes/ascii fields require a positive size", fieldPath + ".size");
                continue;
            }
        ...
        if (getString(fieldJson, "endian", value)) {
            ...
            if (field.endian == Endian::None) {
                addDiagnostic(diagnostics, "BM_SCHEMA_FIELD_ENDIAN_INVALID", "field.endian must be 'little' or 'big'", fieldPath + ".endian");
                continue;
            }
        }
        if ((field.kind == FieldKind::SignedInteger || field.kind == FieldKind::UnsignedInteger) &&
            field.size > 1 && field.endian == Endian::None) {
            addDiagnostic(diagnostics, "BM_SCHEMA_ENDIAN_REQUIRED", "multi-byte integer fields require explicit schema or field endianness", fieldPath + ".endian");
            continue;
        }
        if (field.size > kMaxBinaryBytes) {
            addDiagnostic(diagnostics, "BM_SCHEMA_FIELD_SIZE_LIMIT", "field size exceeds policy limit", fieldPath + ".size");
            continue;
        }

        std::uint64_t next = 0;
        if (!checkedAdd(field.offset, field.size, next)) {
            addDiagnostic(diagnostics, "BM_LAYOUT_RANGE_OVERFLOW", "field offset plus size overflows", fieldPath + ".offset");
            continue;
        }
    ...
    if (schema.fields.empty() && diagnostics.empty()) {
        addDiagnostic(diagnostics, "BM_SCHEMA_FIELDS_EMPTY", "schema must define at least one field", fieldsPath);
    }
...
class BinaryView {
public:
    explicit BinaryView(std::vector<std::uint8_t> bytes) : bytes_(std::move(bytes)) {}
...
void writeOutput(const std::string& path, const std::string& payload) {
    if (path.empty() || path == "-") {
        std::cout << payload;
        return;
    }
...
void printUsage(std::ostream& out) {
    out << "Usage:\n"
        << "  binmorph inspect --schema <schema.json> --input <binary|-> [--output <out.json|->]\n\n"
        << "Barebones schema dialect:\n"
        << "  {\"dialect\":\"binmorph.schema.v1\",\"endianness\":\"little\",\"root\":{\"type\":\"struct\",\"fields\":[...]}}\n";
}
...
int run(int argc, char** argv) {
    ...
    Json schemaJson;
    try {
        schemaJson = JsonParser(readTextFile(options.schemaPath)).parse();
    } catch (const std::exception& error) {
        addDiagnostic(diagnostics, "BM_SCHEMA_JSON_INVALID", error.what(), "$");
        writeOutput(options.outputPath, renderDiagnosticOnly(std::move(diagnostics)));
        return 1;
    }
    ...
    std::vector<std::uint8_t> bytes;
    try {
        bytes = readBinaryFile(options.inputPath);
    } catch (const std::exception& error) {
        ...
        result.diagnostics = std::move(diagnostics);
        writeOutput(options.outputPath, renderResult(result));
        return 1;
    }
