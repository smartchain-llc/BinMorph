...
namespace test_utils
{
  ...
  namespace data
  {
     ...
     */
    static uint8_t Simple[10] = {0x01, 0x02, 0x00, 0xff, 0x01,
  ...
  {
    return std::filesystem::exists(name) ? std::filesystem::path{name}
  ...
  // static auto CreateAndWrite = [](const std::filesystem::path &filePath,
  // uint8_t *data, const std::size_t &len)
  // {
...
#include <functional>
  ...
  {
    explicit WriteStrategy(
        std::function<void(const std::size_t &i, uint8_t &c)> writer)
        : _m_writer{std::move(writer)} {}
  ...
  struct GenerateBinaryData
  {
    ...
    void operator+=(WriteStrategy<WriteLength> &DataWriter)
    {
      ...
      {
        std::cerr << "DataWriter write length (" << WriteLength
                  << ") exceeds buffer size  (" << Length << ")\n";
        return;
      }
      if (WriteLength + _m_write_i > Length)
      {
        _m_write_i = Length - WriteLength;
        std::cerr << "WriteStrategy write length would exceed buffer "
                     "size\nWriting from byte position: "
                  << _m_write_i << "\n";
      }
